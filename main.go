...
func main() {
	...
	fmt.Println("waiting")
	<-wait
	fmt.Println("wait closed consuming data now")
...
// used to satisfy the CopyFromSource interface for avoiding
// buffering all the data in memory
type copyFromSource struct {
	// both of the channels below are used for communicating
	// with the upstream process

	// used to indicate when an error happened
	errorC <-chan error
...
func parseFlags() (int64, int64) {
	...
	flag.Int64Var(&timeout, "timeout", 2_0, "timeout in minutes")
	flag.Parse()
